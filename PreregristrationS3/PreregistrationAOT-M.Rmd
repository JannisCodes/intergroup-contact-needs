...
1. \label{hyp:contact}Based on the most general understanding of the contact hypothesis, an increase in frequency and quality of contact should jointly account for changes in more favorable outgroup attitudes.
    ...
    b. \label{hyp:contactDummy}Outgroup attitudes should be more positive after an intergroup interaction compared to a non-outgroup interaction.
...
3. \label{hyp:keyNeed}Based on our proposal, intergroup interactions with higher situational core need fulfillment should predict more favorable outgroup attitudes due to more positive interaction quality perceptions.
    ...
    c. \label{hyp:keyNeedMediation}The variance explained in outgroup attitudes by key need fulfillment should to a large extend be explained by interaction quality.
...
1. Contact Hypothesis (using between participant aggregates only to meaningfully include interaction frequency)
    ...
    a. T-test: $\mu_{i,0utgroupInteraction} > \mu_{i,IngroupInteraction}$ (to build confidence in the persistence of the contact effect, we additionally compare the attitudes after an outgroup interaction to attitudes after no outgroup interaction (i.e., including instances where participants had no interaction at all).)
    a. Regression: $Attitude = ContactFreq \times AverageContactQual$
1. Allport's Conditions
    a. Regression: 
    $Attitude_{ti} = \gamma_{00} + \gamma_{10}AllportConditions_{ti} + u_{0i} + u_{1i} + e_{ti}$
    a. Regression:
    \begin{equation} \label{eq:SlopesAttAllportQual}
      \begin{split}
        \textrm{Level 1:} &\ Attitude_{ti} = \beta_{0i} + \beta_{1i}AllportConditions_{ti} + \beta_{2i}InteractionQuality_{ti} + e_{ti}\\
    ...
    \begin{flalign}
      ...
      &\textrm{Level 2 Variance:}\ \begin{bmatrix} u_{0i}\\ u_{1i}\\ u_{2i}\end{bmatrix} 
      \sim \mathcal{N}
      \begin{pmatrix}
        \begin{bmatrix} 
          0 \\ 
...
Given the (expectedly) large sample size of contemporaneous measurements, we will remove all instances during which a participant did not respond to any of the relevant variables. Given that we are not necessarily interested in individual trajectories in our analyses, we will use all available data (i.e., will not remove entire participants but rather individual incomplete daily diary measurements). 
