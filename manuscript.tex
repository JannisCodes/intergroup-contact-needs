...
Before we turn to the individual studies, we would like to discuss two methodological matters. Firstly, we opted to test most of our hypotheses using multilevel regression models, where the measurement occasions (level 1) were nested within the participants (level 2). Such an approach is also tolerant to missing data and uneven case numbers within participants. As is common with multilevel analyses, we use a hierarchical modeling approach and always report the final model in-text \citep[][; for the full modeling process see Supplementary Material A]{Bliese2013}. Secondly, statistical power is notoriously difficult to estimate for intensive longitudinal- and multilevel models because of the complex covariance structures. However, for all studies both the number of participants and the number of measurements are within the highest percentiles of sample sizes found within the broader intensive longitudinal literature \citep[e.g.,][]{AanhetRot2012}. Additionally, power simulations after the first study showed that our data was sufficiently powered for even small effect sizes for the contemporaneous effects we assess here. The sample sizes were only increased to allow for between-subjects comparisons and future, more complicated, time-series modeling.
