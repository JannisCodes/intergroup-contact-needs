...
# order time
  # move this here? 
  # date.period.order <- unique(raw$dat.daily$date_period)
  date.period.order <- .GlobalEnv$date.period.order # <- @Max: Do you still need this? Order should be fine natively from FormR

  df$TID <- factor(df$date_period, levels = date.period.order)
...
#                                       Combine Variables
# -------------------------------------------------------------------------------------------------------------
source("lib/fun.panel.R")
# Relatedness
  .GlobalEnv$pairs.panels.new(df[c("RelatednessSelf","RelatednessOther")], 
...
# -------------------------------------------------------------------------------------------------------------
createTimeDf <- function(tmp) {
# make dataframe for all variables that need t-1
# @Max: LAGS AREN'T REALLY MEANINGFUL BECAUSE OF THE TIME INTERVAL DIFFERENCES, WE WILL DO THIS IN DSEM (MPLUS).
  tmp <- data.frame(KeyNeedFullfillment = tmp$KeyNeedFullfillment,
                    ProSo = tmp$ProSo,
                    AntiSo = tmp$AntiSo,
                    AttitudesDutch = tmp$AttitudesDutch)
    length <- ncol(tmp) # get the length of tmp for later renaming of columns
  
  # use dplyr to create time series
    tmp <- tmp %>% mutate_each(funs(lag(.,1))) %>% cbind(tmp, .)
    colnames(tmp)[(length+1):ncol(tmp)] <- paste(colnames(tmp)[1:length], "Lag1", sep = "") #rename after specific length (add one so new columns get renamed after that)
  tmp
}  

# split df per participants and run function
  PP <- split(df, df$PID)
  PP <- lapply(PP, createTimeDf); rm(createTimeDf)
  
# add variables back to df
  tmp <- do.call(rbind.data.frame, PP)
    colnames(tmp) <- paste(colnames(tmp), "Calc", sep = "")
df <- cbind(tmp, df);rm(tmp)
...
df <- cbind(remergePP, df);rm(remergePP)
print("Test whether newly calculate df and old df have the same order:")
print(table(as.character(df$CC_Calc) == as.character(df$CC)))
print("If we also add the NAs we get the exact number:")
...
# -------------------------------------------------------------------------------------------------------------
  # center within participant
  # @Max: I removed the lagged variables so we can do it proberly in MPlus
    df$ProSo_c <- ave(df$ProSoCalc, df$PID, FUN=function(x) scale(x, center = T, scale = F))
    #df$ProSoLag1_c <- ave(df$ProSoLag1Calc, df$PID, FUN=function(x) scale(x, center = T, scale = F))
    df$AntiSo_c <- ave(df$AntiSoCalc, df$PID, FUN=function(x) scale(x, center = T, scale = F))
    #df$AntiSoLag1_c <- ave(df$AntiSoLag1Calc, df$PID, FUN=function(x) scale(x, center = T, scale = F))
    df$KeyNeedFullfillment_c <- ave(df$KeyNeedFullfillmentCalc, df$PID, FUN=function(x) scale(x, center = T, scale = F))
    #df$KeyNeedFullfillmentLag1_c <- ave(df$KeyNeedFullfillmentLag1Calc, df$PID, FUN=function(x) scale(x, center = T, scale = F))
    
    df$closeness_c <- ave(df$closeness_Calc, df$PID, FUN=function(x) scale(x, center = T, scale = F))
    
# variancec
  df$var.AntiSo <- ave(df$AntiSo, df$PID, FUN=var)

# send back to global environment
  .GlobalEnv$df.full = df
