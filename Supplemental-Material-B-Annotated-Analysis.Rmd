...
mdlWorker$lmeInterceptAttCoreInt <-
  lme(
    thermometerDutch_1 ~ keyMotiveFulfilled * c,
    random =  ~ 1 | PID,
    data = workerInteractionType
  )
...
## With ML structure 
# get predicted values and calculate SE
plot.int <-
  sjPlot::get_model_data(
    ...
    terms = c(
      "OutgroupInteraction",
      paste("keyMotiveFulfilled [",paste(seq(-50, 50, 10), collapse = ", "),"]",collapse = "")
    ),
    pretty = F
  )
plot.int$se = (plot.int$conf.high - plot.int$predicted)/1.96
  
# plot predicted values with SE band
ggplot(plot.int,
       aes(
         x = as.numeric(as.character(group)),
         y = predicted,
         group = as.factor(x)
       )) +
  geom_line(aes(linetype = as.factor(x)), size = 1) +
  #geom_point(aes(shape=as.factor(x)), size=2, position = position_dodge(0.0)) +
  geom_ribbon(aes(ymin = predicted - se, ymax = predicted + se), alpha =
                0.2) +
  scale_y_continuous(
    limits = c(50, 100),
    breaks = seq(50, 100, by = 10),
  ...
  scale_linetype_manual(values = c("dashed", "solid")) +
  xlab("Fulfillment Relatedness") +
  ylab("Outgroup Attitudes") +
  guides(
    shape = guide_legend(title = "Intergroup Contact"),
    linetype = guide_legend(title = "Intergroup Contact")
  ) +
  theme_bw() +
  theme(legend.position = "bottom",
        legend.key.size = unit(1, "cm"))

#### Predict Standard errors with Options ####
  # create parameters for prediction
mdlWorker$predInterceptAttCoreInt <- 
  workerOutgroupInteraction %>%
  select(PID, thermometerDutch_1, OutgroupInteraction) %>% 
  mutate(measure = predict(mdlWorker$lmerInterceptAttCoreInt,
                           workerOutgroupInteraction,
                           re.form = NA
                           )
         )
...
datNew = data.frame(
  OutgroupInteraction = rep(c("No", "Yes"), each = 21),
...
PI <-
  merTools::predictInterval(
    merMod = mdlWorker$lmerInterceptAttCoreInt,
    newdata = datNew,
    level = 0.95,
    n.sims = 10000,
    stat = "mean",
...
dat.mod = cbind(datNew, PI)
dat.mod$se = (dat.mod$upr - dat.mod$fit) / 1.96
...
ggplot(dat.mod,
  ...
  #geom_ribbon(data=dat.mod,aes(ymin=lwr,ymax=upr),alpha=0.3) +
  geom_line(aes(linetype = as.factor(Contact_dum.f)), size = 1) +
  #facet_wrap(~PID, ncol = 6)+
  geom_ribbon(data = dat.mod,
              aes(ymin = fit - se, ymax = fit + se),
              alpha = 0.3) +
  scale_linetype_manual(values = c("dashed", "solid")) +
  xlab("Fulfilment Relatedness") +
  ylab("Outgroup Attitudes") +
  guides(
    color = guide_legend(title = "Intergroup Contact"),
    linetype = guide_legend(title = "Intergroup Contact")
  ) +
  theme_Publication()
...
fitavg <-
  cbind(datNew2,
        fitted(brmfit, newdata = datNew2, re_formula = NA)[, -2])
...
ggplot(workerInteractionType, aes(x = keyMotiveFulfilled, y = thermometerDutch_1)) +
...
Also check key need fulfillment generally.
