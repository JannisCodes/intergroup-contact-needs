...
t(1200) = 4.36, \textit{p} = \textless{} .001,
\textbackslash textit\{95\%CI{[}1.36, 3.59{]}), even after controlling
for having an interaction with a non-Dutch (which did not relate to
outgroup attitudes independently; For full results see Online
...
t(365) = 2.93, \textit{p} = = 0.004,
\textbackslash textit\{95\%CI{[}0.06, 0.30{]}) and also predicted higher
perceived interaction quality (random intercept model; \textit{b} =
0.42, t(365) = 8.07, \textit{p} = \textless{} .001,
\textbackslash textit\{95\%CI{[}0.32, 0.52{]}). However, if we consider
the influences of core need fulfillment and interaction quality on
...
\textit{b} = 0.23, t(364) = 4.22, \textit{p} = \textless{} .001,
\textbackslash textit\{95\%CI{[}0.12, 0.33{]}) and only little unique
variance is still explained by core need fulfillment (\textit{b} = 0.04,
t(364) = 0.64, \textit{p} = = 0.523,
\textbackslash textit\{95\%CI{[}-0.08, 0.15{]}). We thus find support
for our hypotheses and can conclude that in this data set the
fulfillment of core situational needs had a significant influence on
outgroup attitudes and that this effect is likely explained by its
effect through perceived interaction quality.
