...
library(XML)
df <- data.frame(readHTMLTable(htmlParse(tabModelTest))[1])
...
tab_model(
  mdlWorker$lmerInterceptAttTypeTbl,
  mdlStudent$lmerSlopesAttType,
  mdlMedical$lmerSlopesAttType
)
...
for (i in 1:length(mdlPrepGeneral)){
  mdlTblGeneral <- join(mdlTblGeneral, mdlPrepGeneral[[i]]$mdlTbl, by = "coef")
}

mdlTblHeaderGeneral <- data.frame(
  ...
  mutate(header = paste(study, pred))

mdlTblHeaderGeneralFreq <- rbind.data.frame(rle(mdlTblHeaderGeneral$header)) %>%
...
mdlTblHeaderGeneral2 <- data.frame(names = mdlTblHeaderGeneral$dv, span = 2) %>% 
  mutate(names = as.character(names)) %>%
  rbind(c("", 1), .) %>%
  mutate(span = as.numeric(span))

mdlTblHeaderGeneral2.2 <- rbind.data.frame(rle(mdlTblHeaderGeneral$dv)) %>%
  mutate(lengths = lengths*2) %>%
  rename(names = values, span = lengths) %>%
  rbind(c(1, ""), .) %>%
  mutate(span = as.numeric(span)) %>%
...
mdlTblGeneral %>%
  ...
  kbl(
    .,
    caption = "Intergroup Contact General within PPT.",
    #format = "latex",
    linesep = "",
    booktabs = T,
    align = c("l", rep("c", ncol(.)-1)),
  ...
  kable_classic() %>%
  add_header_above(., mdlTblHeaderGeneral2) %>%
  pack_rows("Fixed", 1, 3) %>%
  pack_rows("Random", 4, 8) %>%
  pack_rows("Fit", 9, 21)
...
tab_model(
  # Study 1:
  tblLmerS1QltInterceptCore,
  tblLmerS1AttSlopeCore,
  tblLmerS1AttSlopeCoreQlt,
  # Study 2:
  tblLmerS2QltSlopesCore,
  tblLmerS2AttSlopesCore,
  tblLmerS2AttSlopesCoreQlt,
  # Study 3:
  tblLmerS3QltSlopesCore,
  tblLmerS3AttSlopesCore,
  tblLmerS3AttSlopesCoreQlt, 
  tblLmerS3QltSlopesAllport, 
  tblLmerS3AttSlopesAllport,
  tblLmerS3AttSlopesAllportQlt,
  tblLmerS3AttSlopesAllportCore,
  # Settings
  show.std = TRUE,
  show.re.var = TRUE
)
...
mdlTbl$coef <- mapvalues(mdlTbl$coef, 
          from = c("sigmaSq", "tau00", "tau11", "rho01", "rho00", "R2_marginal", "R2_conditional"), 
          to = c("$\\sigma^2$", "$\\tau_{00}$", "$\\tau_{11}$", "$\\rho_{01}$", "$\\rho_{00}$", "$R^2_{marginal}$", "$R^2_{conditional}$"))
...
sapply(mdlRobustPrep, "[[", 1)
sapply(mdlRobustPrep, function(x) x[1]$coeficients$coef)

mdlRobustPrep[[1]]$coeficients$coef
