...
The full surveys are available in our Online Supplementary Material A and the full data description is available in Online Supplementary Materials B. Correlations and descriptive statistics of the included variables are available in Table \ref{tab:descrFullLong} and Table \ref{tab:descrOutLong}.
...
The full surveys are available in our Online Supplementary Material A and the full data description is available in Online Supplementary Materials B. Correlations and descriptive statistics of the included variables are available in Tables \ref{tab:descrFullLong} and \ref{tab:descrOutLong}.
...
for(i in 1:length(var)){
    ...
    png(file = "Figures/forestParametricGeneralLm.png", height = 200 + 40*k^.85) 
      ...
      m=1
      for(i in 3:1){
        addpoly(meta.parametric[[i]], row=m, cex=0.75, mlab=paste("►FE",names[i]),)
        m <- m+6
      }
...
### Bootstrapped Meta-Analysis
    ...
    png(file = "Figures/forestBootstrappedGeneralLm.png", height = 200 + 40*k^.85)
      ...
      m=1
      for(i in 3:1){
        addpoly(meta.bootstrapped[[i]], row=m, cex=0.75, mlab=paste("►FE",names.boot[i]),)
        m <- m+6
      }
      text(-2.05, 19, "Predictors by study",  cex=.8, pos=4)
...
for (i in 1:length(mdlRobustTblElements)){
  ...
  mutate(
    ...
    m = nrow(.), 
    dv = gsub("Raw", "", str_extract(lmerTblPrep(mdlRobustTblElements[[i]][[1]], mdlRobustTblElements[[i]][[2]])$fit$formula,".+?(?= ~)")),
    sample = gsub("S", "Study", str_extract(lmerTblPrep(mdlRobustTblElements[[i]][[1]], mdlRobustTblElements[[i]][[2]])$fit$data, "S1|S2|S3")),
