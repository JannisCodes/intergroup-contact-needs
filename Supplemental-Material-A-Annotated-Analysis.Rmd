...
# goal-directedness coding
goal_topics <- read.csv("data/goal_topics/outgroup_interactions-all_ppt_goals_with_topics.csv")
goal_directedness_coding <- read.csv("data/goal_topics/outgroup_interaction_need_topics_goal_directedness_coding.csv")
...
between <- df %>%
  ...
  mutate(
    ...
    AvAccidental = mean(interaction_accidental+50, na.rm = TRUE),
    AvVoluntary = mean(interaction_voluntary+50, na.rm = TRUE),
...
workerInteractionType <- dtWorker$full %>%
  ...
  mutate(
    interaction_accidental = interaction_accidental+50,
    interaction_voluntary = interaction_voluntary+50
  ) %>%
  group_by(PID) %>%
  mutate(
  ...
  ungroup %>%
  mutate(
    keyMotiveFulfilled = keyMotiveFulfilled+50,
    keymotive_fulfillemt_1 = keymotive_fulfillemt_1+50,
    competence.daily.all = competence.daily.all+50,
    autonomy.daily.all = autonomy.daily.all+50,
    relatedness.daily.all = relatedness.daily.all+50,
    competence_1 = competence_1+50,
    autonomy_1 = autonomy_1+50,
    relatedness_1 = relatedness_1+50,
    quality_overall_1 = quality_overall_1+50,
    interaction_quality = interaction_quality+50,
    ExWB_1 = ExWB_1+50,
    OutgroupInteractionLab = as.character(OutgroupInteraction)
  )
...
studentContactFreq <- dtStudents$full %>%
  ...
  summarise(
    ...
    AvQualityComb = mean(interaction_quality, na.rm = TRUE),
    AvQualityCombOut = mean(interaction_quality[InteractionDumDutch==1], na.rm = TRUE),
...
2. \label{hyp:keyNeed}Based on our proposal, intergroup interactions with higher situational need fulfillment should predict more favorable outgroup attitudes due to more positive interaction quality perceptions.
    a. \label{hyp:keyNeedPred}Outgroup attitudes should be more favorable after intergroup interactions with high situational need fulfillment.
    b. \label{hyp:keyNeedQual}Interaction Quality should be perceived as more positive after intergroup interactions with higher situational need fulfillment.
    c. \label{hyp:keyNeedMediation}The variance explained in outgroup attitudes by situational need fulfillment should to a large extend be assumed by interaction quality.
    d. \label{hyp:keyNeedContactType}The effect of situational need fulfillment on outgroup attitudes should be specific to intergroup interactions and not be due to need fulfillment in general. Thus, the effect of situational need fulfillment on outgroup attitudes should stronger for intergroup interact than for ingroup interactions.
    e. \label{hyp:keyNeedSDT}The effect of situational need fulfillment on outgroup attitudes should be persist even when taking other fundamental psychological needs into account. Thus, the effect of situational need fulfillment on outgroup attitudes should remain strong even after controlling for autonomy, competence, and relatedness fulfillment during the interaction (cf., self-determination theory). 
...
ggplot(
  ...
  labs(
    ...
    caption = "Note: Situational Need Fulfillment is on a 0 to 100 scale, whereas all other scales are on a -50 to 50 scale."
  ) + 
  theme(
...
workerMlCor <-
  MlCorMat(
    data = workerInteractionType,
    id = "PID",
    selection = c("keyMotiveFulfilled", "competence.daily.all", "autonomy.daily.all", "relatedness.daily.all", "interaction_quality", "interaction_accidental", "interaction_voluntary", "thermometerDutch_1"),
    labels = c("Situational Need", "Competence", "Autonomy", "Relatedness", "Quality", "Accidental", "Voluntary", "Attitudes NL")
  ) 
...
workerAvFreQual <- dtWorker$full %>%
  ...
  summarise(
    ...
    AvQualityComb = mean(interaction_quality, na.rm = TRUE),
    AvQualityCombOut = mean(interaction_quality[Contact_dum==1], na.rm = TRUE)
  ) %>%
  mutate(
    WinSumContactNL = DescTools::Winsorize(SumContactNL),
    WinSumContactNLAll = DescTools::Winsorize(SumContactNLAll),
    ...
    AvQualityCombOut_c = AvQualityCombOut - mean(AvQualityCombOut, na.rm = TRUE)
  )
```
...
# correlation panel
pairs.panels.new(
  workerAvFreQual %>% select(SumContactNL, SumContactNLAll, AvQualityComb, AvAttitude),
  labels = c(
    "Sum:\nNumer of beeps with Outgroup Contact (NL)",
    "Sum:\nNumber of Outgroup Contacts (NL)",
    "Mean:\nInteraction Quality",
...
# correlation panel with interaction sums winsorized
pairs.panels.new(
  workerAvFreQual %>% select(WinSumContactNL, WinSumContactNLAll, AvQualityComb, AvAttitude),
  labels = c(
    "Sum:\nNumer of beeps with Outgroup Contact (NL)\n[Winsorized]",
    "Sum:\nNumber of Outgroup Contacts (NL)\n[Winsorized]",
    "Mean:\nInteraction Quality",
...
mdlWorker$lmAttFreqQualX <-
  lm(AvAttitude ~ SumContactNL_c * AvQualityCombOut_c, data = workerAvFreQual)
...
The main proposal of our article is that the success of an outgroup interaction might be explained by whether or not the interaction fulfilled the person's situational need. This should, in turn, be due to a higher perceived interaction quality. We will this sequentially test whether the fulfillment of the situational need during an interaction is (1) related to more positive outgroup attitudes, (2) higher perceived interaction quality, and (3) whether the variance explained by the situational need is assumed by the perceived interaction quality if considered jointly.     

### Need fulfillment and Attitudes  

In a first step we, thus, test the relationship between outgroup attitudes and the fulfillment of the situational need during the interaction.  
...
(\#eq:workerOutInterceptAttNeedEq)
  \begin{split}
    \textrm{Level 1:} &
      \begin{aligned}[t]
        \ Attitude_{ti} =  &\ \beta_{0i} + \beta_{1i}SitNeedFulfill_{ti} + e_{ti}
      \end{aligned} \\
    \textrm{Level 2:} &\ \beta_{0i} = \gamma_{00} + u_{0i} \\
                      &\ \beta_{1i} = \gamma_{10}
  \end{split} 
...
ggplot(data = workerOutWithinBetween,
  ...
  labs(y = "Outgroup Attitudes", 
       x = "Situational Need Fulfillment",
       ) + # rename axes
...
We find that adding the random slopes does add significantly beyond the random intercept model. We also find that the situational need remains a strong predictor (even when letting the influence vary between participants).  

TL;DR: The random slope adds significantly to the prediction model.  

### Need fulfillment and Interaction Quality   

Based on the assertion that the relationship between situational need fulfillment and outgroup attitudes is explained by a higher perceived interaction, the situational need fulfillement should also significantly predict perceived interaction quality.  
...
(\#eq:workerOutInterceptQltNeedEq)
  \begin{split}
    \textrm{Level 1:} &
      \begin{aligned}[t]
        \ InteractionQuality_{ti} =  &\ \beta_{0i} + \beta_{1i}SitNeedFulfill_{ti} + e_{ti}
      \end{aligned} \\
    \textrm{Level 2:} &\ \beta_{0i} = \gamma_{00} + u_{0i} \\
                      &\ \beta_{1i} = \gamma_{10}
  \end{split} 
...
summ(
  mdlWorkerOut$Qlt$lmerInterceptCore <- 
    lmer(interaction_quality ~ keymotive_fulfillemt_1_cwc + (1 | PID), 
       data = workerOutWithinBetween),
  confint = TRUE,
  digits = 3,
  center = FALSE
...
The predictor again adds a significant amount of explained variances beyond the empty model and looking at the slope coefficient, we find that the situational need fulifillment relates significantly to perceived interaction quality. The situational need explained substantial variance between participants (`r format(round(mdlWorkerOut$Qlt$varBtwPercInterceptCore, 2),nsmall=2)`%). The variance explained within participants is also medium (`r format(round(mdlWorkerOut$Qlt$varWithinPercInterceptCore, 2),nsmall=2)`%).  

#### random slope

As before, we check whether further letting the effect of situational need fulfillment vary between participants explains additional variance in outgroup attitudes (i.e., random slope).   
...
ggplot(data = workerOutWithinBetween,
       aes(y = interaction_quality, x = keymotive_fulfillemt_1, col = as.factor(PID))) +
  geom_point(size = 1) + # change size and colour
...
We find that adding the random slopes does not add significantly beyond the random intercept model. This is unusual because this might indicate the the effect is very consistent across participants. However, we also see that when taking the possibility to varying slopes into account, the situational need fulfillment remains a significant predictor of perceived interaction quality.  

### Interaction Needs, Quality, and Attitudes  

In our final main step, we will jointly consider the effect of situational need fulfillment and perceived interaction quality on outgroup attitudes. We expect that if the relationship between situational need fulfillment and outgroup attitudes is indeed explained by a higher perceived interaction quality, the interaction quality perception should assume the explained variance of the situational contact need fulfillment.  

#### random intercept with level one predictors  

We thus add both the situational need fulfillment and perceived interaction quality to a random intercept model of outgroup attitudes.  
...
(\#eq:workerOutInterceptAttQltNeedEq)
  \begin{split}
    \textrm{Level 1:} &
      \begin{aligned}[t]
        \ Attitude_{ti} =  &\ \beta_{0i} + \beta_{1i}SitNeedFulfill_{ti} + \\
...
To make certain that the effect of situational need fulfillment is specific to the interaction we compare the the effect to fulfillment of the situational need when no outgroup interaction took place.  

##### random intercept   

Here we go back to the full dataset and add generalized situational need fulfillment (either during an interaction or about the daytime in general) and whether an outgroup interaction happened as well as their interaction term to a random intercept model of outgroup attitudes.  
    ...
    \textrm{Level 1:} &
      \begin{aligned}[t]
        \ Attitude_{ti} =  &\ \beta_{0i} + \beta_{1i}SitNeedFulfill_{ti} + \\
                           ...
                           &\ \beta_{3i}SitNeedFulfillXOutgroupInteraction_{ti} + e_{ti}
      \end{aligned} \\
    \textrm{Level 2:} &\ \beta_{0i} = \gamma_{00} + u_{0i} \\
                      &\ \beta_{1i} = \gamma_{10} \\
                      &\ \beta_{2i} = \gamma_{20} \\
...
We find that the model explains significantly more variance than the empty null model. However, more interestingly, looking at the coefficients, we find that, as seen earlier, having an outgroup interaction has a strong effect on outgroup attitudes. Importantly, we find that there is no main effect of situational need fulfillment but a significant interaction effect of situational need fulfillment and outgroup contact. This indicates that it is not situational need fulfillment in general --- but only situational need fulfillment during an outgroup contact that predicts more positive outgroup attitudes.  
...
We find that adding the random slopes does add significantly beyond the random intercept model. We also see that when taking the possibility to varying slopes into account, the coefficient interpretations remains consistent (i.e., outgroup contact and its interaction with situational need fulfillment remain significant predictors of positive outgroup attitudes).  

##### Plot Interaction   

Before we move on, we shortly illustrate the interaction effect of how the effect of situational need fulfillment differed by whether an outgroup contact took place or not. To this end we illustrate (1) the raw data points (without taking the nested nature into account), as well as a plot of the model predicted values and their prediction interval (taking the nested structure into account based; similar to an interaction plot).  
...
ggplot(data = workerInteractionType,
  ...
  labs(
    ...
    x = "Fulfillment Situational Need",
...
In a final step we check whether during the interaction the situational need is a meaningful predictor even when taking other fundamental psychological needs into account. We focus on the three commonly considered self determination needs: competence, autonomy, and relatedness.  

##### random intercept with level oe predictors  

We add the situational need fulfillment with the three self determination needs to a random intercept model of outgroup attitudes.  
...
We compare the models of the situational need and the SDT need fulfillments to a model that only includes the SDT needs. We find that the situational need adds significantly above the SDT needs. We find that next to relatedness, the situational need explains the most variance and compared to the model with only the SDT needs, the situational need fulfillment flexibly takes on some of the explained variance of all of the three fundamental needs (i.e., reduction in SDT beta weights). 
...
We find that adding the random slopes does add significantly beyond the random intercept model. We also see that when taking the possibility to varying slopes into account, the coefficient interpretations remains consistent (i.e., situational need and relatedness remain the strongest and only significant predictors). Note however a slight reduction in the p value of the situational need fulfillment.   


#### Interaction Accidental  

To test whether the effect of situational need fulfillment is relevant to planned and accidental interactions (i.e., whether 'goal-directed'), we compare the the effect to fulfillment of the situational need when interactions were accidental or planned.  

##### random intercept   

Here we use the interaction dataset and add generalized situational need fulfillment and whether the interaction was planned or accidental as well as their interaction term to a random intercept model of outgroup attitudes.  
    ...
    \textrm{Level 1:} &
      \begin{aligned}[t]
        \ Attitude_{ti} =  &\ \beta_{0i} + \beta_{1i}SitNeedFulfill_{ti} + \\
                           ...
                           &\ \beta_{3i}SitNeedFulfillXInteractionAccidental_{ti} + e_{ti}
      \end{aligned} \\
    \textrm{Level 2:} &\ \beta_{0i} = \gamma_{00} + u_{0i} \\
                      &\ \beta_{1i} = \gamma_{10} \\
                      &\ \beta_{2i} = \gamma_{20} \\
...
interactions::interact_plot(
  ...
  interval = TRUE,
  partial.residuals = TRUE,
  x.label = "Situational Need Fulfillment",
...
Here we use the interaction dataset and add generalized situational need fulfillment as well as interaction quality in the most complex model.
...
(\#eq:worderOutSlopeWbNeedQualEq)
  \begin{split}
    \textrm{Level 1:} &
      \begin{aligned}[t]
        \ Well-Being_{ti} =  &\ \beta_{0i} + \beta_{1i}SitNeedFulfill_{ti} + \\
...
workerWbModels <-
  list(
    ...
    wb_need_qual = ExWB_1 ~ keymotive_fulfillemt_1_cwc + interaction_quality_cwc
  )
workerWbModelType <-
  list(
    intercept = ~ 1 | PID,
    slope_need = ~ 1 + keymotive_fulfillemt_1_cwc | PID,
    slope_need_qual = ~ 1 + keymotive_fulfillemt_1_cwc + interaction_quality_cwc | PID
  )
...
summ(
  mdlWorkerOut$Wb$lmerSlopeNeedQual <- lmer(
    ExWB_1 ~ keymotive_fulfillemt_1_cwc + interaction_quality_cwc + (1 + keymotive_fulfillemt_1_cwc + interaction_quality_cwc | PID),
    data = workerOutWithinBetween
  ),
  confint = TRUE,
  digits = 3,
...
summ(
  mdlWorkerOut$Wb$lmerSlopeNeedQualZ <- lmer(
    ExWB_1_zwc ~ keymotive_fulfillemt_1_zwc + interaction_quality_zwc + (1 + keymotive_fulfillemt_1_zwc + interaction_quality_zwc | PID),
    data = workerOutWithinBetween
  ),
  confint = TRUE,
  digits = 3,
...
studentDescr <- studentInteractionType %>%
  select(
    ...
    `Well-being` = ExWB, 
    `Situational Need` = KeyNeedFullfillment,
...
studentMlCor <-
  MlCorMat(
    data = studentInteractionType,
    id = "PID",
    selection = c("KeyNeedFullfillment", "Competence", "Autonomy", "Relatedness", "interaction_quality", "interaction_accidental", "interaction_voluntary", "AttitudesDutch"),
    labels = c("Situational Need", "Competence", "Autonomy", "Relatedness", "Quality", "Accidental", "Voluntary", "Attitudes NL")
  ) 
...
# correlation panel
pairs.panels.new(
  studentContactFreq %>% select(SumContactNL, SumContactNLAll, AvQualityComb, AvAttitude),
  labels = c(
    "Sum:\nNumer of beeps with Outgroup Contact (NL)",
    "Sum:\nNumber of Outgroup Contacts (NL)",
    "Mean:\nInteraction Quality",
...
# correlation panel with interaction sums winsorized
pairs.panels.new(
  studentContactFreq %>% select(WinSumContactNL, WinSumContactNLAll, AvQualityComb, AvAttitude),
  labels = c(
    "Sum:\nNumer of beeps with Outgroup Contact (NL)\n[Winsorized]",
    "Sum:\nNumber of Outgroup Contacts (NL)\n[Winsorized]",
    "Mean:\nInteraction Quality",
...
mdlStudent$lmAttFreqQualX <-
  lm(AvAttitude ~ SumContactNL_c * AvQualityCombOut_c, data = studentContactFreq)
...
Based on the assertion that the relationship between situational need fulfillment and outgroup attitudes is explained by a higher perceived interaction, the situational need fulfillment should also significantly predict perceived interaction quality.  
...
summ(
  mdlStudentOut$Qlt$lmerInterceptCore <- 
    lmer(interaction_quality ~ KeyNeedFullfillment_cwc + (1 | PID), 
       data = studentOutWithinBetween),
  confint = TRUE,
  digits = 3,
  center = FALSE
...
The predictor again adds a significant amount of explained variances beyond the empty model and looking at the slope coefficient, we find that the situational need fulfillment relates significantly to perceived interaction quality. The situational need explained substantial variance between participants (`r format(round(mdlStudentOut$Qlt$varBtwPercInterceptCore, 2),nsmall=2)`%). The variance explained within participants is also medium (`r format(round(mdlStudentOut$Qlt$varWithinPercInterceptCore, 2),nsmall=2)`%).  
...
ggplot(data = studentOutWithinBetween,
       aes(y = interaction_quality, x = KeyNeedFullfillment, group = as.factor(PID))) +
  geom_point(size = 1, alpha = .2, position = position_jitter(width = 3, height = 3)) + # change size and colour
  labs(y = "Interaction Quality", 
       x = "Situational Need Fulfillment",
       ) + # rename axes
...
Unsurprisingly, the model with the two predictors adds significantly beyond the empty unconditional model. However, more importantly, looking at the coefficients, we find that the effect of situational need fulfillment indeed is indeed strongly reduced and the variance is explained by the perceived interaction quality. The variance explained in outgroup attitudes is of medium effect size (between: `r format(round(mdlStudentOut$Att$varBtwPercInterceptCoreQlt,2),nsmall=2)`%, within: `r format(round(mdlStudentOut$Att$varWithinPercInterceptCoreQlt,2),nsmall=2)`%).  
...
We find that the model explains significantly more variance than the empty null model. However, more interestingly, looking at the coefficients, we find that, as seen earlier, having an outgroup interaction has a strong effect on outgroup attitudes. Importantly, we find that there is a main effect of situational need fulfillment but also a significant interaction effect of situational need fulfillment and outgroup contact. This indicates that it is not simply situational need fulfillment in general --- but especially situational need fulfillment during an outgroup contact that predicts more positive outgroup attitudes.  
...
We find that adding the random slopes does add significantly beyond the random intercept model. We also see that when taking the possibility to varying slopes into account, the coefficient interpretations remains consistent (i.e., outgroup contact and its interaction with situational need fulfillment remain important predictors of positive outgroup attitudes).  
...
The plots indicate that especially once we take the nested data structure into account we can see a substantially stronger effect of situational need fulfillment on outgroup attitudes during outgroup contacts than without outgroup contacts.  
...
We find that the the model with the added predictor indeed explains more variance in outgroup attitudes than the unconditional model and we find that adding the situational need adds further explained variance --- beyond the self determination needs. Looking at the individual coefficients, we find that the situational need relates significantly to outgroup attitudes, that it is a stronger predictor than any of the self determination theory needs and that it assumes some of the variance explained by the self determination theory needs (when compared to a model without the situational need). 
...
# However, removing eithe the situational need, Autonomy, or Relatedness from the random slopes lets the model converge
...
We find that adding the random slopes does add significantly beyond the random intercept model. We also see that when taking the possibility to varying slopes into account, the coefficient interpretations remains consistent (i.e., situational need and relatedness remain the strongest and only significant predictors).   
...
studentWbModels <-
  list(
    ...
    wb_need_qual = ExWB ~ KeyNeedFullfillment_cwc + interaction_quality_cwc
  )
studentWbModelType <-
  list(
    intercept = ~ 1 | PID,
    slope_need = ~ 1 + KeyNeedFullfillment_cwc | PID,
    slope_need_qual = ~ 1 + KeyNeedFullfillment_cwc + interaction_quality_cwc | PID
  )
...
summ(
  mdlStudentOut$Wb$lmerInterceptNeedQuality <- lmer(
    ExWB ~ KeyNeedFullfillment_cwc + interaction_quality_cwc + (1 | PID),
    data = studentOutWithinBetween
  ),
  confint = TRUE,
  digits = 3,
...
summ(
  mdlStudentOut$Wb$lmerInterceptNeedQualityZ <- lmer(
    ExWB_zwc ~ KeyNeedFullfillment_zwc + interaction_quality_zwc + (1 | PID),
    data = studentOutWithinBetween
  ),
  confint = TRUE,
  digits = 3,
...
medicalDescr <- dtMedical$full %>%
  transmute(
    Attitudes = AttitudesDutch,
    `Well-being` = exWB, 
    `Situational Need` = KeyNeedFulfillment,
    Competence = Competence,
    Autonomy = Autonomy,
    Relatedness = Relatedness,
...
medicalMlCor <-
  MlCorMat(
    data = dtMedical$full,
    id = "PID",
    selection = c("KeyNeedFulfillment", "Competence", "Autonomy", "Relatedness", "AllportsCondition", "interaction_quality", "interaction_accidental", "interaction_voluntary", "AttitudesDutch", "exWB"),
    labels = c("Situational Need", "Competence", "Autonomy", "Relatedness", "Allport", "Quality", "Accidental", "Voluntary", "Attitudes NL", "Well-being")
  ) 
...
# correlation panel
pairs.panels.new(
  medicalContactFreq %>% select(SumContactNL, SumContactNLAll, AvQualityComb, AvAttitude),
  labels = c(
    "Sum:\nNumer of beeps with Outgroup Contact (NL)",
    "Sum:\nNumber of Outgroup Contacts (NL)",
    "Mean:\nInteraction Quality",
...
# correlation panel with interaction sums winsorized
pairs.panels.new(
  medicalContactFreq %>% select(WinSumContactNL, WinSumContactNLAll, AvQualityComb, AvAttitude),
  labels = c(
    "Sum:\nNumer of beeps with Outgroup Contact (NL)\n[Winsorized]",
    "Sum:\nNumber of Outgroup Contacts (NL)\n[Winsorized]",
    "Mean:\nInteraction Quality",
...
mdlMedical$lmAttFreqQualX <-
  lm(AvAttitude ~ SumContactNL_c * AvQualityCombOut_c, data = medicalContactFreq)
...
We begin our main analysis by outgroup attitudes after an intergroup contact are indeed explained by whether or not Allport's conditions were fulfilled. This should, in turn, be due to a higher perceived interaction quality. We will this sequentially test whether the fulfillment of the situational need during an interaction is (1) related to more positive outgroup attitudes, (2) higher perceived interaction quality, and (3) whether the variance explained by the situational need is assumed by the perceived interaction quality if considered jointly.     
...
ggplot(data = medicalOutWithinBetween,
       aes(y = interaction_quality, x = AllportsCondition, group = as.factor(PID))) +
  geom_point(size = 1, alpha = .15, position = position_jitter(width = 3, height = 3)) + # change size and colour
  labs(y = "Interaction Quality", 
       x = "Fulfillment Allport's Conditions",
       ) + # rename axes
...
Unsurprisingly, the model with the two predictors adds significantly beyond the empty unconditional model. However, more importantly, looking at the coefficients, we find that the effect of Allport's conditions is indeed is indeed strongly reduced and the variance is explained by the perceived interaction quality. Notably in this sample, the reduction of variance explained is not a much as we saw with situational need fulfillment in previous studies.  
...
We find that the the model with the added predictor indeed explains more variance in outgroup attitudes than the unconditional model. Looking at the individual coefficients, we find that the situational need relates significantly to outgroup attitudes.  
...
The predictor again adds a significant amount of explained variances beyond the empty model and looking at the slope coefficient, we find that the situational need fulfillment relates significantly to perceived interaction quality. 
...
ggplot(data = medicalOutWithinBetween,
       aes(y = interaction_quality, x = KeyNeedFulfillment, group = as.factor(PID))) +
  geom_point(size = 1, alpha = .15, position = position_jitter(width = 3, height = 3)) + # change size and colour
  labs(y = "Interaction Quality", 
       x = "Keyneed Fulfillment",
       ) + # rename axes
...
We find that adding the random slopes does add significantly beyond the random intercept model and situation situational need fulfillment remains a meaningful predictor of outgroup attitudes.   

### Interaction Needs, Quality, and Attitudes  

In our final main step, we will jointly consider the effect of situational need fulfillment and perceived interaction quality on outgroup attitudes. We expect that if the relationship between situational need fulfillment and outgroup attitudes is indeed explained by a higher perceived interaction quality, the interaction quality perception should assume the explained variance of the core contact need fulfillment.  
...
The random slopes add significant explained variance to the model. When we consider the influences of situational need fulfillment and interaction quality on outgroup attitudes jointly, we find that perceived interaction quality is a substantially stronger predictor and the unique variance explained by situational need fulfillment was roughly half of its original effect size.

## Compare Fulfillment of Situational Need and Allport's Conditions

To compare the models using either Allport's conditions or the situational need fulfillment to predict outgroup attitudes, we first assess relative model performance indices (i.e., Akaike information criterion, and Bayesian information criterion), and then consider the two predictors in a joint model to see whether the two approaches predict the same variance in outgroup attitudes.
...
When comparing the model selection indices we find that the fulfillment of the situational need, indeed performs slightly better than the model using Allport's conditions. 
...
We begin by adding both the situational need fulfillment and Allport's conditions to a random intercept model of outgroup attitudes.  
...
We find that the model explains significantly more variance than the empty null model. However, more interestingly, looking at the coefficients, we find that, as seen earlier, having an outgroup interaction has a strong effect on outgroup attitudes. Importantly, we find that there is no main effect of situational need fulfillment by itself but the effect is qualified by a significant interaction effect of situational need fulfillment and outgroup contact. This indicates that it is not simply situational need fulfillment in general --- but especially situational need fulfillment during an outgroup contact that predicts more positive outgroup attitudes.  
...
medicalWbModels <-
  list(
    ...
    wb_need_qual = exWB ~ KeyNeedFulfillment_cwc + interaction_quality_cwc
  )
medicalWbModelType <-
  list(
    intercept = ~ 1 | PID,
    slope_need = ~ 1 + KeyNeedFulfillment_cwc | PID,
    slope_need_qual = ~ 1 + KeyNeedFulfillment_cwc + interaction_quality_cwc | PID
  )
...
summ(
  mdlMedicalOut$Wb$lmerSlopeNeedQual <- lmer(
    exWB ~ KeyNeedFulfillment_cwc + interaction_quality_cwc + (1 + KeyNeedFulfillment_cwc + interaction_quality_cwc | PID),
    data = medicalOutWithinBetween
  ),
  confint = TRUE,
  digits = 3,
...
summ(
  mdlMedicalOut$Wb$lmerSlopeNeedQualZ <- lmer(
    exWB_zwc ~ KeyNeedFulfillment_zwc + interaction_quality_zwc + (1 + KeyNeedFulfillment_zwc + interaction_quality_zwc | PID),
    data = medicalOutWithinBetween
  ),
  confint = TRUE,
  digits = 3,
...
tblDtS1Needs <- workerOutWithinBetween %>%
  select(
    ...
    QualityRaw = interaction_quality,
    Quality = interaction_quality_cwc,
    QualityZ = interaction_quality_zwc,
    PID = PID
  )
# Quality ~ CoreNeed
...
TheoryMetaQualityNeed <-
  metaLmerOut(
    ...
    title = "Quality ~ Situational Need"
  )
...
TheoryMetaAttitudeNeed <-
  metaLmerOut(
    ...
    title = "Attitude ~ Situational Need"
  )
...
TheoryMetaAttitudeNeedQuality <-
  metaLmerOut(
    ...
    title = "Attitude ~ Situational Need + Quality"
  )
```

![Situational Need Fulfillment predicting Interaction Quality.](Figures/forestParametricFETheoryQualityCore.png)

![Situational Need Fulfillment predicting Outgroup Attitudes.](Figures/forestParametricFETheoryAttitudeCore.png)

![Situational Need Fulfillment and Interaction Quality predicting Outgroup Attitudes.](Figures/forestParametricFETheoryAttitudeCoreQuality.png)
...
![Situational Need Fulfillment predicting Outgroup Attitudes, while controlling for self-determination theory needs (intergroup contact sample).](Figures/forestParametricFERobustSDT.png)


# **Robustness: Goal-Directedness across Studies**  

```{r}
#| label: topic_data_prep

# prep general data
dt_goal_s1 <- workerOutWithinBetween %>%
  transmute(
    study = as.numeric(1),
    PID = PID,
    TIDnum = as.numeric(TID),
    TID = paste(PPDate, str_to_title(daytime)),
    OutgroupInteraction = OutgroupInteraction,
    NonOutgroupInteraction = NonOutgroupInteraction,
    Attitude = thermometerDutch_1,
    AttitudeC = thermometerDutch_1_zwc,
    AttitudeZ = thermometerDutch_1_zwc,
    CoreNeed = keymotive_fulfillemt_1,
    CoreNeedC = keymotive_fulfillemt_1_cwc,
    CoreNeedZ = keymotive_fulfillemt_1_zwc,
    InteractionQuality = interaction_quality,
    InteractionQualityC = interaction_quality_cwc,
    InteractionQualityZ = interaction_quality_zwc,
    Quality = interaction_quality,
    QualityC = interaction_quality_cwc,
    QualityZ = interaction_quality_zwc,
    WellBeing = ExWB_1,
    WellBeingC = ExWB_1_cwc,
    WellBeingZ = ExWB_1_zwc,
    InteractionAccidental = interaction_accidental,
    InteractionAccidentalC = interaction_accidental_cwc,
    InteractionAccidentalZ = interaction_accidental_zwc,
    Competence = competence_1,
    CompetenceC = competence_1_cwc,
    CompetenceZ = competence_1_zwc,
    Autonomy = autonomy_1,
    AutonomyC = autonomy_1_cwc,
    AutonomyZ = autonomy_1_zwc,
    Relatednesss = relatedness_1,
    RelatednesssC = relatedness_1_cwc,
    RelatednesssZ = relatedness_1_zwc,
    motive = keyMotive
  )
dt_goal_s2 <- studentOutWithinBetween %>%
  transmute(
    study = as.numeric(2),
    PID = PID,
    TIDnum = TIDnum,
    ...
    NonOutgroupInteraction = NonOutgroupInteraction,
    Attitude = AttitudesDutch,
    AttitudeC = AttitudesDutch_cwc,
    AttitudeZ = AttitudesDutch_zwc,
    CoreNeed = KeyNeedFullfillment,
    CoreNeedC = KeyNeedFullfillment_cwc,
    CoreNeedZ = KeyNeedFullfillment_zwc,
    InteractionQuality = interaction_quality,
    InteractionQualityC = interaction_quality_cwc,
    InteractionQualityZ = interaction_quality_zwc,
    ...
    InteractionAccidentalZ = interaction_accidental_zwc,
    Competence = Competence,
    CompetenceC = Competence_cwc,
    CompetenceZ = Competence_zwc,
    Autonomy = Autonomy,
    AutonomyC = Autonomy_cwc,
    AutonomyZ = Autonomy_zwc,
    Relatednesss = Relatedness,
    RelatednesssC = Relatedness_cwc,
    RelatednesssZ = Relatedness_zwc,
    motive = KeyNeed
  )
dt_goal_s3 <- medicalOutWithinBetween %>%
  transmute(
    study = as.numeric(3),
    PID = PID,
    TIDnum = TIDnum,
    ...
    NonOutgroupInteraction = NonOutgroupInteraction,
    Attitude = AttitudesDutch,
    AttutudeC = AttitudesDutch_cwc,
    AttitudeZ = AttitudesDutch_zwc,
    CoreNeed = KeyNeedFulfillment,
    CoreNeedC = KeyNeedFulfillment_cwc,
    CoreNeedZ = KeyNeedFulfillment_zwc,
    Allport = AllportsCondition,
    AllportC = AllportsCondition_cwc,
    AllportZ = AllportsCondition_zwc,
    InteractionQuality = interaction_quality,
    InteractionQualityC = interaction_quality_cwc,
    InteractionQualityZ = interaction_quality_zwc,
...
dt_goal_all <- bind_rows(list(dt_goal_s1,
                              dt_goal_s2,
                              dt_goal_s3)) %>%
  filter(motive != "") %>%
  filter(!is.na(motive))
...
# topic coding for export
dt_topic_coding <- goal_directedness_coding

# prep goal data
dt_goal_coding <- goal_topics %>%
  left_join(
    goal_directedness_coding %>% select(-top_n_words, -starts_with("representative_")),
    by = "topic"
  ) %>%
  mutate(
    study = as.numeric(gsub("S", "", study)),
    PID = as.numeric(PID)
  ) %>% 
  as_tibble

# join goal data to ESM data
dt_goal_joined <- 
  dt_goal_all %>%
  left_join(
    dt_goal_coding %>% mutate(motive = text),
    keep = FALSE,
    # pretty messy merge because the export excluded empty and duplicate motives but should work
    by = c("motive", "study", "PID"), #"motive",
    multiple = "first"
  ) %>%
  filter(!is.na(text)) # remove unmatched
```

### Goal-Directedness Distribution

```{r}
#| label: goal_distribution

dt_goal_directedness <- 
  dt_goal_joined %>%
  # filter(study == 2) %>% 
  # remove after full coding
  mutate(
    practical_need_dum = ifelse(practical_need == 2, 1, practical_need)-.5,
    psychological_need_dum = ifelse(psychological_need == 2, 1, psychological_need)-.5,
  )
rm(
  dt_goal_all,
  dt_goal_coding,
...
lab_mapping <- c("no need", "vague need", "clear need")
goal_directedness_dist <- dt_goal_directedness %>%
  ...
  mutate(
    practical_need_label = lab_mapping[match(practical_need, c(0, 1, 2))],
    psychological_need_label = lab_mapping[match(psychological_need, c(0, 1, 2))]
  ) %>%
  group_by(
    practical_need,
...
ggplot(
  ...
  aes (
    x = reorder(practical_need_label, practical_need),
    y = reorder(psychological_need_label, psychological_need),
    fill = count,
...
```{r}
goal_directedness_model <-
  list(
    null = Attitude ~ 1,
    wb_need = Attitude ~ CoreNeedC,
    wb_need_direct = Attitude ~ CoreNeedC * practical_need_dum + CoreNeedC * psychological_need_dum
  )
goal_directedness_model_type <-
  list(
    intercept = ~ 1 | PID,
    slope_need = ~ 1 + CoreNeedC | PID,
    slope_need_direct = ~ 1 + CoreNeedC + practical_need_dum + psychological_need_dum | PID
  )

# Goal Directedness Null Model
lmeNull <-
  lme(goal_directedness_model$null,
      random = goal_directedness_model_type$intercept,
      na.action = na.omit,
      data = dt_goal_directedness)
# Well-being ~ Need
lmeInterceptNeed <-
  lme(goal_directedness_model$wb_need,
      random = goal_directedness_model_type$intercept,
      na.action = na.omit,
      data = dt_goal_directedness)
lmeSlopeNeed <-
  lme(goal_directedness_model$wb_need,
      random = goal_directedness_model_type$slope_need,
      na.action = na.omit,
      data = dt_goal_directedness)
# lmeSlopeDirect <-
#   lme(goal_directedness_model$wb_need_direct,
#       random = goal_directedness_model_type$slope_need_direct,
#       na.action = na.omit,
#       control = lmeControl(opt = "optim", maxIter = 100, msMaxIter = 100),
#       data = df)
...
summ(lmer(
  Quality ~ CoreNeedC * practical_need_dum + CoreNeedC * psychological_need_dum + 
    (1 + CoreNeedC + practical_need_dum + psychological_need_dum | study/PID), #study/PID
  data = dt_goal_directedness
))

summ(
  lmer(
    QualityZ ~ CoreNeedZ * practical_need_dum + CoreNeedZ * psychological_need_dum + 
      (1 + CoreNeedZ + practical_need_dum + psychological_need_dum | study/PID), #study/PID
    data = dt_goal_directedness
  )
)
```
...
summ(lmer(
  Attitude ~ CoreNeedC * practical_need_dum + CoreNeedC * psychological_need_dum + 
    (1 + CoreNeedC + practical_need_dum + psychological_need_dum | study/PID), #study/PID
  data = dt_goal_directedness
))
...
summ(
  lmer(
    AttitudeZ ~ CoreNeedZ * practical_need_dum + CoreNeedZ * psychological_need_dum + 
      (1 + CoreNeedZ + practical_need_dum + psychological_need_dum | study/PID), #study/PID
    data = dt_goal_directedness
  )
)
...
summ(lmer(
  Quality ~ CoreNeedC * practical_need_dum + CoreNeedC * psychological_need_dum + 
    (1 + CoreNeedC + practical_need_dum + psychological_need_dum | PID), #study/PID
...
summ(
  lmer(
    QualityZ ~ CoreNeedZ * practical_need_dum + CoreNeedZ * psychological_need_dum + 
      (1 + CoreNeedZ + practical_need_dum + psychological_need_dum | PID), #study/PID
    data = dt_goal_directedness %>% filter(study == 1)
  )
)
```
...
# **Robustness: General Contact Hypothesis Test across Studies**  
...
allMlS1 <- workerInteractionType %>%
  select(
    ...
    qualityOverall = interaction_quality,
    Competence = competence.daily.all, 
    Autonomy = autonomy.daily.all, 
    Relatedness = relatedness.daily.all
  ) %>%
...
# correlation panel
pairs.panels.new(
  avAll %>% select(SumContactNL, SumContactNLAll, AvQualityCombOut, AvAttitude),
  labels = c(
    "Sum:\nNumer of beeps with Outgroup Contact (NL)",
    "Sum:\nNumber of Outgroup Contacts (NL)",
    "Mean:\nInteraction Quality",
...
# correlation panel with interaction sums winsorized
pairs.panels.new(
  avAll %>% select(WinSumContactNL, WinSumContactNLAll, AvQualityCombOut, AvAttitude),
  labels = c(
    "Sum:\nNumer of beeps with Outgroup Contact (NL)\n[Winsorized]",
    "Sum:\nNumber of Outgroup Contacts (NL)\n[Winsorized]",
    "Mean:\nInteraction Quality",
...
dt_exp_s1 <- workerWithinBetween %>%
  transmute(
    ...
    OutgroupInteraction = OutgroupInteraction,
    NonOutgroupInteraction = NonOutgroupInteraction,
    OutgroupInteractionLab = OutgroupInteractionLab,
    Attitude = thermometerDutch_1,
    AttitudeC = thermometerDutch_1_zwc,
    AttitudeZ = thermometerDutch_1_zwc,
    CoreNeed = keyMotiveFulfilled,
    CoreNeedC = keyMotiveFulfilled_cwc,
    CoreNeedZ = keyMotiveFulfilled_zwc,
    InteractionQuality = interaction_quality,
    InteractionQualityC = interaction_quality_cwc,
    InteractionQualityZ = interaction_quality_zwc,
    Quality = interaction_quality,
    ...
    InteractionAccidentalZ = interaction_accidental_zwc,
    Competence = competence.daily.all,
    CompetenceC = competence.daily.all_cwc,
    CompetenceZ = competence.daily.all_zwc,
    Autonomy = autonomy.daily.all,
    AutonomyC = autonomy.daily.all_cwc,
    AutonomyZ = autonomy.daily.all_zwc,
    Relatednesss = relatedness.daily.all,
    RelatednesssC = relatedness.daily.all_cwc,
    RelatednesssZ = relatedness.daily.all_zwc,
    motive = keyMotive
  )
dt_exp_s2 <- studentWithinBetween %>%
  transmute(
    study = as.numeric(2),
    PID = PID,
    TIDnum = TIDnum,
...
dt_exp_s3 <- medicalWithinBetween %>%
  transmute(
    study = as.numeric(3),
    PID = PID,
    TIDnum = TIDnum,
...
dt_exp_all <- bind_rows(list(dt_exp_s1,
                             dt_exp_s2,
                             dt_exp_s3))
rm(dt_exp_s1,
   dt_exp_s2,
